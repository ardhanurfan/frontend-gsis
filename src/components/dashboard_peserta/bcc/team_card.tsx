...
const TeamCard = ({ status, hidden, state, link, row }: TeamCardProps) => {
  ...
  return (
    <div
      className={
        status
          ? "mt-[20px] mx-auto w-[89.744%] max-w-[1204px] border-2 border-primaryBlue rounded-lg p-5 flex flex-col gap-[20px]"
          : "hidden"
      }
    >
      <div className="flex justify-between items-center">
        <div className="flex flex-col gap-[5px]">
          <h3 className="header3 text-primaryBlue">Team Name</h3>
          <p className="body-text text-primaryBlue">{row.team_name}</p>
        </div>
        <a
          href="/document-gsic"
          className={
            hidden
              ? "hidden"
              : "bg-primaryBlue hover:bg-seccondaryBlue flex justify-center items-center w-[20%] max-w-[134px] h-[30px] lg:h-[40px] lg:button-text button-text-mobile text-white rounded-lg"
          }
        >
          Edit
        </a>
      ...
      <div className="flex flex-col lg:flex-row justify-between gap-[10px]">
        ...
        <a
          href={link}
          className="bg-primaryBlue hover:bg-seccondaryBlue flex justify-center items-center w-auto px-10 h-[25px] lg:h-[40px] button-text-mobile lg:button-text text-white rounded-lg"
        >
